...
`v0.6.4` 基线验证：Swift 6 严格警告构建通过，114 项 XCTest 全部通过。标签页已迁移到 AppKit 原生 `NSWindow` 标签组，由系统提供标签外观、新建按钮、排序与拆分窗口交互，并保留新标签的位置、视图模式和排序状态。回归范围覆盖 My Computer 首页、位置快捷方式侧边栏、iCloud Drive、Launch Services 文件夹打开、窗口布局与白屏回归、视图切换后的选择保持、目录读取与取消、稳定文件 ID、Spotlight 与递归搜索、变化监听、挂载卷、剪贴板、file promise 拖放类型与文件 URL 优先级，以及文件操作的字节进度、永久删除、批量冲突、回滚、Undo/Redo 往返、队列取消和符号链接边界安全。
