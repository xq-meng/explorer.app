...
| M1 只读浏览 | 进行中 | 异步目录加载、稳定文件 ID、取消、可点击表头排序、隐藏文件设置、真实导航与挂载卷、懒加载且跟随路径的目录树 | 10 万项目性能报告和更多网络卷验证 |
| M2 视图与标签页 | 进行中 | 详细信息/图标视图、按可见范围生成的 Quick Look 缩略图与有界内存缓存、多标签页、独立历史与选择、可点击/可编辑面包屑、标签路径/选中标签/视图模式/排序恢复、自定义收藏夹、窗口内标签拖动 | 最近访问、缩略图磁盘缓存策略、跨窗口标签拖动、滚动位置和完整历史恢复 |
| M3 文件操作 | 进行中 | 安全操作引擎、串行队列、进度事件、回滚式替换、剪贴板、菜单/右键操作、安全 Undo/Redo | 字节级进度、批量冲突交互、用户可见取消、持久化操作历史、`NSFileCoordinator` 全面接入 |
| M4 系统集成 | 进行中 | Quick Look 缩略图、浮动面板和可切换右侧预览窗格，当前目录搜索、递归搜索兜底、目录变化监听、普通文件 URL 的应用内及跨应用拖放 | Spotlight 后端、file promise 与跨应用兼容性矩阵验证 |
| M5 发布准备 | 进行中 | 本地开发 `.app` 组装、原创多分辨率应用图标、Info.plist、entitlements、ad-hoc 签名与签名验证、GitHub 预发布与 Homebrew Cask 自动发布 | Developer ID、沙盒授权、公证、DMG、应用内自动更新、辅助功能和兼容性矩阵 |

当前代码以“可构建、测试通过的开发原型”为目标；尚未达到本文定义的 Beta 退出条件。

本轮架构重构后基线验证：Swift 6 Debug 构建通过；精简后的 43 项 XCTest 覆盖窗口内容布局与白屏回归、懒加载目录树、目录读取与取消、稳定文件 ID、搜索与变化监听、挂载卷、剪贴板，以及文件操作的冲突、回滚、Undo/Redo 往返、队列取消和符号链接边界安全。已移除仅验证框架参数或无状态幂等调用的低价值测试。
...
Explorer.app 的首要目标不是复刻 Windows 界面素材，而是复用其高效率的文件管理方式：目录树、地址栏、详细信息列表、标签页、键盘操作、文件操作队列和冲突处理。
...
- 支持目录树、地址栏、标签页、详细信息和图标两种视图。
- 支持新建、重命名、复制、移动、剪切、粘贴、拖放和移入废纸篓。
- 支持 Quick Look、当前目录搜索和外部文件变化刷新。
- 所有文件 I/O 均不阻塞主线程。
...
- `AppKit`：窗口、目录树、列表、图标网格、菜单、键盘和拖放
